"""An Investigation of Markov Chain Monte Carlo Methods: sin(X).

Employs the antithetic variable and control variate approaches to reduce the
variance of the estimate of the integral of sin(X) from 0 to 1, with X = X1 and
X = X1...X10.
...
plt.xlim(2., n)
plt.ylabel(r"$\bar{f}$")
plt.title(r"$\int_0^1\sin(x)\,dx$")
plt.xlabel(r"$n$")
plt.legend()
plt.savefig("sin(X1).pdf", bbox_inches='tight')
plt.clf()
...
plt.xlabel(r"$n$")
plt.ylabel(r"$\bar{f}$")
plt.title(r"$\int_0^1\ldots\int_0^1 \sin(x_1\ldots x_{10})\,dx_1\ldots dx_{10}$")
plt.legend()
plt.savefig("sin(X1...X10).pdf", bbox_inches='tight')
plt.show()
